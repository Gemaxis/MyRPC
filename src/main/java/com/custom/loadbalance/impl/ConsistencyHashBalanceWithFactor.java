 ...
 **/
public class ConsistencyHashBalanceWithFactor implements LoadBalance {
    // 真实节点列表
    ...
    static {
        initServer(realNodes);
    }

    private static void initServer(List<Machine> realNodes) {
    ...
    public String balance(List<String> addressList) {
        List<Machine> nodes = addressList.stream()
                .map(address -> new Machine(address, LoadFactor.Memory8G))
                .collect(Collectors.toList());
        initServer(nodes);
        // 随机选择一个客户端节点进行哈希计算和服务器选择
        String random = UUID.randomUUID().toString();
        Machine selectedMachine = getServer(random);
        return selectedMachine != null ? selectedMachine.getHost() : null;
    }
}
